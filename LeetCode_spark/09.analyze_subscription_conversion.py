...
# # # # # #### ================ Approach->1 : (DSL)
free_df=(df.filter(col("activity_type")=="free_trial")
         .groupby("user_id")
         .agg(round(avg("activity_duration"),2).alias("trial_avg_duration"))
         )

paid_df=(df.filter(col("activity_type")=="paid")
         .groupby("user_id")
         .agg(round(avg("activity_duration"),2).alias("paid_avg_duration"))
         )
